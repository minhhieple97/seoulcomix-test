...
export const RestaurantDescription: FC<RestaurantDescriptionProps> = ({ className }) => {
  const { restaurant } = useRestaurantCardContext();
  const desc = restaurant?.desc;
  return <p className={`truncate text-base text-primary md:text-lg ${className}`}>{desc}</p>;
};
